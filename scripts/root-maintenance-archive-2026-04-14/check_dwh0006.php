...
$pelanggan = Pelanggan::where('id_pelanggan', 'DWH0006')->first();

if (!$pelanggan) {
    echo "Pelanggan DWH0006 tidak ditemukan\n";
    exit;
}

echo "=== Data Pelanggan DWH0006 (NENENG) ===\n";
...
if ($tagihan) {
    ...
    echo "  - Meteran Sebelum: " . ($tagihan->meteran_sebelum ?? 'NULL') . "\n";
    echo "  - Meteran Sesudah: " . ($tagihan->meteran_sesudah ?? 'NULL') . "\n";
    echo "  - Pemakaian: " . ($tagihan->pemakaian_kubik ?? 0) . " m³\n";
    echo "  - Tarif per m³: Rp " . number_format($tagihan->tarif_per_kubik, 0, ',', '.') . "\n";
    echo "  - Ada Abunemen: " . ($tagihan->ada_abunemen ? 'Ya' : 'Tidak') . "\n";
    echo "  - Biaya Abunemen: Rp " . number_format($tagihan->biaya_abunemen, 0, ',', '.') . "\n";
